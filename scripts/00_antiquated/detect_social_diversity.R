...
detect_social_diversity <- function(df, id, input){
  ...
  divictionary <- read_csv("diversity_project - h1_dictionary.csv", col_types = cols()) 
  ...
  nonhuman <- readr::read_csv("diversity_project - humanizeR.csv", col_types = cols())
  ...
  tmp_df <- tmp_df %>% 
    ...
    tidytable::mutate.(soc_diversity = if_else(diversity_cnt > 0 & total_cnt > diversity_cnt, diversity_cnt, 0)) %>% 
    # but if diversity is mentioned in the same sentence as any animal term then don't count it as social diversity 
    tidytable::mutate.(soc_diversity = if_else(soc_diversity > 0 & nonhuman > 0, 0, soc_diversity)) %>% 
