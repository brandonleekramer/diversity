...
humanizeR <- function(df, id, input){
  ...
  human_terms <- as.vector(human$terms)
  nonhuman <- dictionary %>% filter(category == "nonhuman")
  nonhuman_terms <- as.vector(nonhuman$terms)
...
polysemeR_v1 <- function(df, id, input){
  ...
  # dictionary 
  setwd("~/git/diversity/data/dictionaries/")
  false_positives <- read_csv("diversity_project - polysemeR1.csv") %>% arrange(heterogeneity)
...
polysemeR_v2 <- function(df, id, input){
  ...
  # dictionary 
  setwd("~/git/diversity/data/dictionaries/")
  false_positives <- read_csv("diversity_project - polysemeR2.csv") %>% arrange(heterogeneity)
  false_positives <- na.omit(false_positives$heterogeneity)
...
detect_social_diversity_v1 <- function(df, id, input, remove_polysemes = TRUE){
  ...
  nonhuman <- readr::read_csv("diversity_project - humanizeR.csv")
  ...
  tmp_df <- df %>% 
    ...
    # run compounder to transform dictionary words 
    #compoundR(!!input) %>% 
    ...
    tidytable::filter.(word %in% c(all_diversity_cats, nonhuman)) %>%
    ...
    tidytable::mutate.(term = ifelse( str_detect(word, paste(c("\\b(?i)(zqx", na.omit(divictionary$population), "zqx)\\b"), collapse = "|")),  "population",  term)) %>% 
    ...
    tidytable::mutate.(population_cnt = ifelse( str_detect( term,  "\\b(population)\\b"),  1,  0)) %>%
  ...
  tmp_df <- tmp_df %>% 
    ...
    # get the intial social_diversity count (before polysemeR1 correction)
...
detect_social_diversity_v2 <- function(df, id, input){
  ...
  nonhuman <- nonhuman %>% filter(category == "nonhuman")
  nonhuman <- na.omit(nonhuman$terms)
  exclusion_terms <- readr::read_csv("diversity_project - polysemeR2.csv")
  exclusion_terms <- na.omit(exclusion_terms$heterogeneity)
...
compoundR_v3 <- function(df, input){
  ...
  # load dictionary 
  setwd("~/git/diversity/data/dictionaries/")
  compound_dictionary <- readr::read_csv("diversity_project - compoundR_v3.csv") %>%
    filter(hyp_test == "h1") %>%  
    dplyr::select(original_string, new_string) %>% tibble::deframe()
...
polysemeR_v3 <- function(df, id, input){
  ...
  # dictionary 
  setwd("~/git/diversity/data/dictionaries/")
  false_positives <- read_csv("diversity_project - polysemeR3.csv") 
  false_positives <- na.omit(false_positives$heterogeneity)
...
detect_social_diversity_v3 <- function(df, id, input, remove_polysemes = TRUE){
  ...
  # load dictionaries 
  setwd("~/git/diversity/data/dictionaries/")
  divictionary <- read_csv("diversity_project - h1_dictionary_v3.csv") 
  all_diversity_cats  <- c(na.omit(divictionary$ancestry), na.omit(divictionary$cultural), 
                           na.omit(divictionary$disability), na.omit(divictionary$diversity), 
                           na.omit(divictionary$equity), na.omit(divictionary$lifecourse), 
                           na.omit(divictionary$migration), na.omit(divictionary$minority), 
  ...
  nonhuman <- readr::read_csv("diversity_project - humanizeR_v3.csv")
  nonhuman <- nonhuman %>% filter(category == "nonhuman")
  nonhuman <- na.omit(nonhuman$terms)
  exclusion_terms <- readr::read_csv("diversity_project - polysemeR3.csv")
  ...
  tmp_df <- df %>% 
    ...
    # run compoundR to transform words  
    #compoundR_v3(!!input) %>% 
