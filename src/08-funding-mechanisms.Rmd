...
population_data <- text_data %>% 
  ...
  mutate(sex_gender = ifelse(test = str_detect(string = abstract, 
                       pattern = "\\b(?i)(man|men|male|males|boy|boys|woman|women|female|females|girl|girls|XX|XY|XXY|sex|sexes|gender|genders|sexed|gendered|transgender|transgendered|transsexual|transsexuals|maletofemale|femaletomale)\\b"), 
                       yes = 1, no = 0)) 
```
...
```
```{r race_ethnic_lm}
summary(glm(race_ethnic ~ nih + nsf + hhs + euro + biotech + foundation + 
              university + other, family = binomial, data = funding_data))
```
```{r race_ethnic_lm}
summary(glm(sex_gender ~ nih + nsf + hhs + euro + biotech + foundation + 
              university + other, family = binomial, data = funding_data))
```
