...
eth_df <- ethnicity_synonym %>% mutate(selected = "ethnicity") %>% top_n(15, similarity) 
gen_df <- gender_synonym %>% mutate(selected = "gender") %>% top_n(15, similarity) 
combined_df <- eth_df %>% bind_rows(gen_df)
combined_df
```

```{r}
combined_df <- rownames(combined_df)
...
```{r}
pmi_svd <- irlba(pmi_matrix, 5, maxit = 500)
