...
h1_ids <- h1_ids %>% 
  as_tidytable() %>%
  #filter(across(ancestry:socdiv), 0) %>% 
  mutate(h1_totals = rowSums(across(ancestry:socdiv))) %>% 
  select(fk_pmid, year, h1_totals, everything())
...
combined_ids <- h1_ids %>% 
  ...
  replace(is.na(.), 0) %>% 
  select(fk_pmid, year, h1_totals, h2_totals, h3_totals, 
         racial, ethnic, diversity, everything()) 
...
combined_ids <- combined_ids %>% as.data.frame()
soc_diversity_ids <- soc_diversity_ids %>% as.data.frame()
bio_diversity_ids <- bio_diversity_ids %>% as.data.frame()
...
dbWriteTable(conn, c("pubmed_2021", "all_diversity_ids_0721"), combined_ids, row.names = FALSE)
dbWriteTable(conn, c("pubmed_2021", "soc_diversity_ids_0721"), soc_diversity_ids, row.names = FALSE)
dbWriteTable(conn, c("pubmed_2021", "bio_diversity_ids_0721"), bio_diversity_ids, row.names = FALSE)
dbDisconnect(conn)
