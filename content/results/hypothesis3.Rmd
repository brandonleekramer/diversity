...
social_diversity_prc <- read_rds("~/git/diversity/data/text_results/h1_results/h1_all_set_prc_trends.rds") %>% 
...
figure_3A <- h3_counts %>% 
  ...
  ggplot() + 
  geom_line(aes(y = n, x = year, colour = term), size = 1.2, stat="identity") + 
  theme_minimal() +
...
figure_2B <- all_pop_prc_counts %>% 
  ...
  ggplot() + 
  geom_line(aes(y = percentage, x = year, colour = term), size = 1.2, stat="identity") +
