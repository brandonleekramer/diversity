...
rm(list = ls())
knitr::opts_knit$set(root.dir = "~/Desktop/Diversity/the-growth-of-diversity")
...
# collapsing race*, ethnicity* and cultural* terminology 
racial_variations <- "\\b(?i)(race|racial|racially)\\b"
ethnic_variations <- "\\b(?i)(ethnic|ethnicity|ethnically)\\b"
cultural_variations <- "\\b(?i)(culture|cultural|culturally)\\b"

# recoding race*, ethnicity* and cultural* terminology
recoded_abstract_data <- abstract_data %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, pattern = racial_variations), 
                       yes = "racial*", no = word)) %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, pattern = ethnic_variations), 
                       yes = "ethnic*", no = term)) %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, pattern = cultural_variations), 
...
word_graph <- ggplot() + geom_line(aes(y = n, x = year, colour = term),
...
us_specific_bigrams <- abstract_data %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word,
                       pattern = "\\b(?i)(caucasian|caucasians|whites)\\b"), yes = "white", no = term)) %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, 
                       pattern = "\\b(?i)(americans)\\b"), yes = "american", no = word)) %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, 
...
us_specific_bigrams %>% 
  ...
  filter(bigram == "african-american" | bigram == "american-indian" | bigram == "native-american" |
           
         bigram == "pacific-islander" | 
         bigram == "mexican-american" ) %>% 
  count(bigram, year, sort = TRUE)

#black <- paste(c("\\b(?i)(caucasian|caucasians|whites)\\b"), collapse = "|")
#american_indian <-  paste(c("\\b(?i)(ethnic|ethnicity|ethnically)\\b"), collapse = "|")
...
```{r examining us population terms over time, fig.width=9.5}
# collapsing race* and ethnicity* terminology 
white <- paste(c("\\b(?i)(caucasian|caucasians|white)\\b"), collapse = "|")
black <- paste(c("\\b(?i)(black|african)\\b"), collapse = "|")
asian <- paste(c("\\b(?i)(asian|asians)\\b"), collapse = "|")
pacific_islander <- paste(c("\\b(?i)(pacific|asians)\\b"), collapse = "|")


recoded_abstract_data <- abstract_data %>% 
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, pattern = white), 
                               yes = "white/caucasian", no = word)) %>%
  mutate(term = ifelse(test = str_detect(string = abstract_data$word, pattern = black), 
                               yes = "black/african-american", no = word))
...
diversity_terms <- recoded_abstract_words %>% 
  filter(year != "2018") %>%
  filter(term == "diversity" |  term == "white" | term == "caucasian" | 
         term == "black" | term = "african-american" ) 
diversity_terms
...
recoded_abstract_data <- abstract_data %>% 
  mutate(recoded_word = ifelse(test = str_detect(string = abstract_data$word, 
                                                 pattern = population_specific), 
...
# breaking apart population-specific words 
...
diversity_terms <- recoded_abstract_words %>% 
  filter(recoded_word == "diversity" | recoded_word == "population" | 
           recoded_word == "population-specific" | recoded_word == "general population terms" | 
           recoded_word == "continental terms" | recoded_word == "linguistic & religious terms" | 
           recoded_word == "national terms" | recoded_word == "south american ethnic groups" | 
           recoded_word == "african ethnic groups" | recoded_word == "north american ethnic groups" | 
           recoded_word == "european ethnic groups" | recoded_word == "asian ethnic groups" |
